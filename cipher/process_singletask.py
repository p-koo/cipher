...
def main(args):
    ...
    wrangle.enforce_constant_size(tf_filtered_path, pos_bed_path, window, compression)
    ...
    pos_seq = wrangle.parse_fasta(pos_fasta_path)

    # filter sequences with absent nucleotides
    pos_seq, _ = wrangle.filter_nonsense_sequences(pos_seq)
    ...
    # parse sequence and chromosome from fasta file
    neg_seq = wrangle.parse_fasta(neg_fasta_path)

    # filter sequences with absent nucleotides
    neg_seq, _ = wrangle.filter_nonsense_sequences(neg_seq)
    ...
    neg_one_hot_gc = wrangle.match_gc(pos_one_hot, neg_one_hot)
