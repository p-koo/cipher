...
def main(model_name, data_path, args):
    ...
    N, L, A = x_train.shape
    model = animal.model(input_shape=(L,4), output_shape=num_labels)
...
if __name__ == "__main__":    
    ...
    parser.add_argument("output_path_prefix", metavar="/PATH/TO/OUTPUT/PLUS_PREFIX", type=str,
...
#                     default=False)
    ...
    parser.add_argument("-lr_factor", metavar="REDUCELRONPLATEAU_FACTOR", type=float, 
                    help="Factor for learning rate reduction on plateau callback (see keras.callbacks.ReduceLROnPlateau())", 
                    default=0.2)
    parser.add_argument("-lr_patience", metavar="REDUCELRONPLATEAU_PATIENCE", type=float, 
                    help="Patience for learning rate reduction on plateau callback (see tensorflow.keras.callbacks.EarlyStopping())", 
                    default=3)
    parser.add_argument("-es_patience", metavar="EARLYSTOPPING_PATIENCE", type=float, 
