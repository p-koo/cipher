...
def main(model_name, data_path, args):
    ...
    # Save fit history
    history_df = pd.DataFrame(list(history.history.items()), columns=['column1', 'column2'])
...
if __name__ == "__main__":    
    ...
    parser = argparse.ArgumentParser(description="Train model on single-task dataset and save outputs.",
    ...
    parser.add_argument('-m', "--model_name", metavar="MODEL_NAME", type=str,
                    help="Model name (from model zoo) to use")
    parser.add_argument('-d', "--data_path", metavar="/PATH/TO/DATA/", type=str,
                    help="Path to HDF5 data file")
    parser.add_argument('-o', "--output_path_prefix", metavar="/PATH/TO/OUTPUT/PLUS_PREFIX", type=str,
                    help="Path (if not current working directory) plus filename prefix for two outputs: weights and history")
    
    parser.add_argument('-bs', "--batch_size", metavar="BATCH_SIZE", type=int, 
...
#                     default=False)
    ...
    parser.add_argument('-f', "--lr_factor", metavar="REDUCELRONPLATEAU_FACTOR", type=float, 
                    help="Factor for learning rate reduction on plateau callback (see keras.callbacks.ReduceLROnPlateau())", 
                    default=0.2)
    parser.add_argument('-lrp', "--lr_patience", metavar="REDUCELRONPLATEAU_PATIENCE", type=float, 
                    help="Patience for learning rate reduction on plateau callback (see tensorflow.keras.callbacks.EarlyStopping())", 
                    default=3)
    parser.add_argument('-esp', "--es_patience", metavar="EARLYSTOPPING_PATIENCE", type=float, 
