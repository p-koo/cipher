...
def multitask_bed_generation(target_beds_file,feature_size=1000,merge_overlap=200,out_prefix='features',
          chrom_lengths_file=None,db_act_file=None,db_bed=None,
        ignore_auxiliary=False,no_db_activity=False,ignore_y=False,):
    ...
    if not target_beds_file: 
      raise Exception("Must provide file labeling the targets and providing BED file paths.")
    ...
    if db_bed is not None:
        ...
        if not no_db_activity:
            if db_act_file is None:
                raise Exception('Must provide both activity table or specify -n if you want to add to an existing database')
            else:
        ...
        for line in peak_bed_in:
            if not line.startswith('#'):
                ...
                if chrom_key not in chrom_outs:
                    chrom_files[chrom_key] = '%s_%s_%s.bed' % (out_prefix, chrom, strand)
        ...
        for line in open(chrom_files[chrom_key],'rt'):
            ...
            if len(open_peaks) == 0:
                ...
                if open_end - merge_overlap <= peak.start:
                    # close open peak
                    mpeaks = merge_peaks(open_peaks,feature_size, merge_overlap, chrom_lengths.get(chrom,None))
